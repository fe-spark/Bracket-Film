...
- **V2 架构精简 (Clean Slate)**：根据“新项目、无冗余”的要求，彻底移除了一切迁移相关的防御性逻辑。删除了 `CleanDuplicateSearchInfo` 及启动时的脏数据清理环节，移除了所有旧 ID 自动关联（ReMap）工具。现在的 V2 系统完全依赖于标准化入库逻辑，确保从数据产生的一刻起就是干净、规范的。
